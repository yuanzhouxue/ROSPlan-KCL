...
#include <rosplan_planning_system/PlanDispatch/CFFPlanDispatcher.h>
...
namespace KCL_rosplan {
	...
	CFFPlanDispatcher::CFFPlanDispatcher(ros::NodeHandle& nh) : PlanDispatcher(nh)  {
	...
	CFFPlanDispatcher::~CFFPlanDispatcher()
	{

	}

	void CFFPlanDispatcher::reset() {
	...
	void CFFPlanDispatcher::planCallback(const rosplan_dispatch_msgs::CFFCompletePlan plan) {
	 ...
	 */
	bool CFFPlanDispatcher::dispatchPlan(double missionStartTime, double planStartTime) {
        ...
        while (ros::ok() && current_action < current_plan.plan.size()) {
...
//			current_action++;
	 ...
	 */
	void CFFPlanDispatcher::feedbackCallback(const rosplan_dispatch_msgs::ActionFeedback::ConstPtr& msg) {
		...
		if(!action_completed[msg->action_id] && msg->status == rosplan_dispatch_msgs::ActionFeedback::ACTION_SUCCEEDED_TO_GOAL_STATE) {
            ...
            current_action = current_plan.edges[current_action].true_son_id;
        }
		...
		if(!action_completed[msg->action_id] && msg->status == rosplan_dispatch_msgs::ActionFeedback::ACTION_FAILED) {
			replan_requested = true;
			action_completed[msg->action_id] = true;
            current_action = current_plan.edges[current_action].false_son_id;
		}
	}
} // close namespace
...
int main(int argc, char **argv) {
    ...
    KCL_rosplan::CFFPlanDispatcher spd(nh);

    // subscribe to planner output
    std::string planTopic = "complete_plan";
    nh.getParam("plan_topic", planTopic);
    ros::Subscriber plan_sub = nh.subscribe(planTopic, 1, &KCL_rosplan::CFFPlanDispatcher::planCallback, &spd);

    std::string feedbackTopic = "action_feedback";
    nh.getParam("action_feedback_topic", feedbackTopic);
    ros::Subscriber feedback_sub = nh.subscribe(feedbackTopic, 1000, &KCL_rosplan::CFFPlanDispatcher::feedbackCallback, &spd);
